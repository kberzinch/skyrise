...
void Set_Lift_Target(int Adjustment = 0) {
	Lift_Target = ((SensorValue[EncoderRight] + (SensorValue[EncoderLeft])) / 2) * 2 + Adjustment;
}
...
task Lift_Stabilizer_Left {
	...
	while(IsStabilizerRunning) {
		...
		error = (Lift_Target - (SensorValue[sensor] + 10));
...
task Lift_Stabilizer_Right {
	...
	int error, previous_error, speed, derivative, integral = 0;
	while(IsStabilizerRunning) {
		if(SensorValue[sensor] > 102 || (SensorValue[sensor] < 10 && !MinOverride)) {
